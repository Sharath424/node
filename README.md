...
1. Install Chocolatey from [here](https://chocolatey.org/install).
  ...
  2. Run ` Set-ExecutionPolicy AllSigned` command.
  3. Run `Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))` to install Chocolatey package manager.
  4. Run `choco` to verify the installation
2. Run `choco install openssl` to install openssl cli tool.
3. Open a new powersheel instance and Run `openssl version` to verify the installation. 
...
<body>
    <div class="container">
        ...
        <form action="/todos" method="post">
            <input class="form-input" type="text" name="todo" placeholder="Add a new TODO" required>
...
<body>
    <div class="container">
        ...
        <form action="/todos/<%= id %>/edit" method="post">
            <input class="form-input" type="text" name="todo" value="<%= todo %>" required>
...
Example 3: Find documents where the "score" field is of type double:

```bash
db.scores.find({ score: { $type: "double" } })
```
...
Example 5: Find documents where the "email" field exists and is of type string:
...
```json
[
  { "year": 2023, "month": 9, "product": "Widget A", "quantity": 10, "price": 20 },
  { "year": 2023, "month": 9, "product": "Widget B", "quantity": 8, "price": 15 },
  // ...
]
```
